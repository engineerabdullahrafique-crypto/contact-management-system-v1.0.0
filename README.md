...
A full stack contact management application built with **Spring Boot** (backend) and **React** (frontend). Users can register, log in, and perform CRUD operations on their contacts. The project includes comprehensive testing, code quality analysis with SonarQube, and is ready for CI/CD integration.
...
- **Quality & Testing**  
  - Unit tests for service and repository layers (JUnit, Mockito)  
...
A Postman collection (ContactManagement.postman_collection.json) is included in the docs folder. Import it to test all endpoints.
